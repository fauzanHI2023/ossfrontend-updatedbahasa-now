...
import {initiativeChildren} from '@/data/data';
import Image from 'next/image';

interface InitiativeChildrenProps {
  tab: string; // props untuk filter tab
}

const InitiativeChildren: React.FC<InitiativeChildrenProps> = ({tab}) => {
  const filteredItems = initiativeChildren.filter((item) => item.tab === tab);
            ...
            <div className="flex flex-col gap-y-4">
              <div className="flex flex-row items-center gap-x-2">
                <Image
                  ...
                  height={60}
                />
                <h4 className="text-sky-600 text-2xl font-semibold w-[14rem] leading-7">
                  {item.title}
                </h4>
              </div>
...
export default InitiativeChildren;
