    ...
    <ul className={`flex flex-col gap-x-6 w-full`}>
      {menuItems.map((item) => (
        <li key={item.id} className="relative group text-base font-normal pb-4">
          <Link
            href={item.url}
            className="flex flex-row sm:justify-center sm:justify-between w-full dark:hover:text-slate-200 dark:text-slate-300 hover:text-slate-700 hover:font-semibold transition duration-200 ease-in text-slate-500 text-base font-semibold"
          >
